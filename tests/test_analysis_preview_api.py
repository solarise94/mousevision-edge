...
def test_analysis_preview_404_when_run_not_ready(preview_client):
    ...
    job = app_mod.job_store.create_job(
        project_id="default",
        cage_id="C1",
...
def test_analysis_preview_returns_jpeg(preview_client, tmp_path: Path):
    ...
    run_dir = Path(app_mod.DEFAULT_OUTPUT) / f"run_20260714_{run_id[:8]}"
    ...
    app_mod.job_store.update(job["job_id"], run_id=run_id, status="completed")
    ...
    status = c.get(f"/api/jobs/{job['job_id']}")
