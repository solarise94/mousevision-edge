...
| 识别成功 | 震动反馈 → 结果区显示箱号 → 写入 `sessionStorage.currentBox` → `/mobile/record` |
| 开灯 | `MediaStreamTrack.applyConstraints({ advanced: [{ torch }] })`；不支持则 toast |
| 相册 | `<input type="file" accept="image/*">` → 解码静态图 |
| 重新扫描 | 清空结果并恢复扫描循环 |
| 手动输入 | 底部 Sheet：箱号文本框 + 确认 |
| 使用帮助 | 弹层说明对准框、开灯、相册与手动输入 |
| 返回 | 首页 |
...
**布局**（白/绿 chrome + 画外控制，避免按钮遮挡体重读数区）

```
┌─────────────────────────────┐
│ ←  录制准备 · C57-023   切换 │  ← 顶栏白底；多摄像头时显示「切换」
├─────────────────────────────┤
│  ░ 浅绿边距（非录像内容）░   │
│  ┌─ 绿边框 9:16 视口 ──┐   │
│  │  Canvas 所见即所得    │   │
...
- 页面背景为浅绿/白，不再用大片黑边；预览卡片用绿色描边。
- 录像内容仍是严格 9:16 Canvas（720×1280）；两侧/上下边距属于 UI chrome，不进入上传视频。
- 快门、计时、提示文案放在底部 `rec-dock`，与体重读数区分离。
...
| 帧率 | 15 fps |
| 码率 | 1.5 Mbps |
| 分片 | 无 timeslice（`start()` 一次成片，避免 Android fMP4 问题） |
