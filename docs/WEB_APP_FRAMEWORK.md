...
1. 手机浏览器调用后置摄像头，将 Canvas 取景画面录像并上传。
2. 视频通过 HTTPS 上传到后端。
...
- 当前录制页不再自动或主动切换到系统相机；后端仍兼容历史 `system` 视频上传。
- 上传进度、排队状态、分析状态与错误提示。
- 最近任务和基础称量报告。
...
手机通过 `http://VM_IP:8766/mobile` 通常不会获得 `getUserMedia()` 实时相机权限；当前录制页需要 HTTPS 和支持网页录像的浏览器。后端仍可接收历史系统相机视频。
